    ...
    public class LqvCauHoisController : Controller
    {
        ...
        public async Task<IActionResult> Create(LqvCauHoi model)
        {
            if (model.LqvBoCauHoiId <= 0)
                ModelState.AddModelError("LqvBoCauHoiId", "Chưa chọn bộ câu hỏi");

            if (string.IsNullOrWhiteSpace(model.LqvNoiDung))
                ModelState.AddModelError("LqvNoiDung", "Nội dung không được rỗng");
        ...
        {
            if (id != model.LqvCauHoiId) return NotFound();
            ...
            var entity = await _context.LqvCauHois.FindAsync(id);
        ...
        // =====================================================
        // LOAD DROPDOWN BỘ CÂU HỎI (DÙNG CHUNG)
        // =====================================================
        private void LoadBoCauHoiDropDown(int? selectedId = null)
        {
            ViewBag.BoCauHois = _context.LqvBoCauHois
                ...
                .Select(x => new SelectListItem
                {
                    ...
                    Text = x.LqvTenBo,
                    Selected = selectedId != null && x.LqvBoCauHoiId == selectedId
                })
                .ToList();
        }
    }
